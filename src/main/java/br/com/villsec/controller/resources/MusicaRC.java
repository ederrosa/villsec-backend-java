...
public class MusicaRC {
	...
	public ResponseEntity<Void> insert(@Valid MusicaDTO objNewDTO,
		...
		URI uri = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}")
				.buildAndExpand(theMusicaServices.insert(new MusicaVHWeb().create(objNewDTO), theMultipartFile).getId())
				.toUri();
	...
	public ResponseEntity<Page<MusicaDTO>> findPage(@RequestParam(value = "page", defaultValue = "0") Integer page,
		...
		Page<Musica> list = theMusicaServices.findAllPage(page, linesPerPage, orderBy, direction);
